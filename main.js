        ...
        if (srt.sortColumn || srt.sortCompareFunction != null) {
            ...
            if (srt.sortCompareFunction != null) {
                ...
                funcs.push(srt.isAscending ? function (a, b, srt) {
                    var aVal = (UIUtils.toString(UIUtils.resolveExpression(a, srt.sortColumn))).toLowerCase();
                    var bVal = (UIUtils.toString(UIUtils.resolveExpression(b, srt.sortColumn))).toLowerCase();
                    if (aVal < bVal)
                        return -1;
                    if (aVal > bVal)
                        return 1;
                ...
                funcs.push(srt.isAscending ? function (a, b, srt) {
                    var aVal = parseFloat(UIUtils.resolveExpression(a, srt.sortColumn));
                    var bVal = parseFloat(UIUtils.resolveExpression(b, srt.sortColumn));
                    if (aVal < bVal)
                        return -1;
                    if (aVal > bVal)
                        return 1;
